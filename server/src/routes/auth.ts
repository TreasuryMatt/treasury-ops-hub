...
function issueToken(user: { id: string; caiaId: string; email: string; displayName: string; role: string }): string {
  return jwt.sign(
    { id: user.id, caiaId: user.caiaId, email: user.email, displayName: user.displayName, role: user.role },
    JWT_SECRET,
...
authRouter.post('/mock-login', async (req: Request, res: Response, next: NextFunction) => {
  ...
  res.json({ data: { token, user: { id: user.id, caiaId: user.caiaId, email: user.email, displayName: user.displayName, role: user.role } } });
});

authRouter.get('/me', requireAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  const user = await prisma.user.findUnique({
    where: { id: req.user!.id },
    select: { id: true, caiaId: true, email: true, displayName: true, role: true, isActive: true },
  });
  if (!user) {
